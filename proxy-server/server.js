...
app.post('/api/content/*', async (req, res) => {
  ...
  try {
    ...
    if (endpoint === '/items') {
      ...
      // Spells endpoint - fetch per class and tag with casting classes
      // This solves the 400 Bad Request issue and adds valuable metadata
      ...
      for (const className of classes) {
        try {
          const classUrl = `${DDB_GAME_DATA_BASE}/spells?class=${className}`;
            ...
            if (Array.isArray(spells)) {
              spells.forEach(spell => {
                ...
                if (spellsMap.has(spellId)) {
                  // Spell already exists, add this class to its casterClasses
                  const existingSpell = spellsMap.get(spellId);
                  if (!existingSpell.casterClasses.includes(className)) {
                    existingSpell.casterClasses.push(className);
                  }
                } else {
                  // New spell, add with initial casterClasses array
                  spellsMap.set(spellId, {
                    ...spell,
                    casterClasses: [className]
                  });
                }
              });
            }
          ...
          console.warn(`Failed to fetch spells for ${className}:`, classError.message);
      ...
      console.log(`Fetched ${data.length} unique spells across ${classes.length} classes`);
